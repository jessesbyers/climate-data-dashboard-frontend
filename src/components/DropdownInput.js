...
class DropdownInput extends Component {
    ...
    componentDidMount(){
        fetch("https://climate-dashboard-api.herokuapp.com/charts")
    ...
    handleDropdownSubmit = (event) => {
        ...
        if (this.chart_url !== "") {
            ...
            this.setState({
                ...
                charts: this.state.charts.filter(chart => this.state.chart_url !== `https://climate-dashboard-api.herokuapp.com/charts/${chart.id}`),
                loadedCharts: this.state.charts.filter(chart => this.state.chart_url === `https://climate-dashboard-api.herokuapp.com/charts/${chart.id}`)
            })
        } else {
            ...
            <Form onSubmit={event => this.handleDropdownSubmit(event)}>
                <Form.Group>
                    <Form.Control as="select" size="lg" value={this.state.chart_url} onChange={event => this.handleDropdownChange(event)}>
                        ...
                        {this.state.charts.map(chart => <option value={`https://climate-dashboard-api.herokuapp.com/charts/${chart.id}`} key={chart.id}>{chart.name}</option>)}
                    </Form.Control>
                </Form.Group>
